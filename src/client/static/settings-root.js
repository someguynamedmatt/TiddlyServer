...
app.run(function($templateCache) {
  var templates = {
    string:
      '<input type="text"      title="{{item.name}}" name="{{item.name}}" ng-disabled="readonly" ng-model="outputs[item.name]"/> <span ng-bind-html="description"></span>',
    number:
      '<input type="number"    title="{{item.name}}" name="{{item.name}}" ng-disabled="readonly" ng-model="outputs[item.name]"/> <span ng-bind-html="description"></span>',
    boolean:
      '<input type="checkbox" title="{{item.name}}" name="{{item.name}}" ng-disabled="readonly" ng-model="outputs[item.name]"/> <span ng-bind-html="description"></span>',
    ifenabled:
...
app.controller('HashmapEnumItemCtrl', function($scope, $sce) {
  ...
  if (typeof $scope.outputs[parentItem.name] !== 'object') $scope.outputs[parentItem.name] = {}
...
app.controller('SettingsPageItemCtrl', function($scope, $sce) {
  ...
  if (typeof $scope.description === 'string') {
    ...
    if ($scope.description._) $scope.description._ = $sce.trustAsHtml($scope.description._)
  }
...
app.controller('SettingsPageCtrl', function($scope, $http) {
  ...
  $http.get('?action=getdata').then(function(res) {
    ...
    $scope.$watch(
      function(s) {
        return JSON.stringify(s.outputs)
      },
      function(item, old) {
